...
function safeParseJson(raw) {
  if (!raw) return null
  try {
    return JSON.parse(raw.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '').trim())
  } catch { return null }
}
    ...
    try {
      ...
      for (const m of conv.messages) {
        if (m.role === 'user') {
          ...
          rebuilt.push({ type: 'response', data: parsed
            ? { ...parsed, talk_id: null, video_requested: false }
            : { explanation: m.content, analogy: '', video_script: null, follow_up_questions: [], talk_id: null, video_requested: false }
          })
        }
      }
      setMessages(rebuilt)
  ...
  const handleSend = async (concept) => {
    ...
    try {
      ...
      if (!conversationId) {
        ...
        setConvTitle(concept.slice(0, 60))
      }

      setMessages(prev => [...prev, { type: 'response', data }])

      // Save to localStorage so sidebar shows it
      saveToHistory({
        id:           data.conversation_id,
        title:        convTitle || concept.slice(0, 60),
      ...
      setMessages(prev => [...prev, { type: 'error', message: err.message, retryText: concept }])
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px' }}>
            <div style={{ maxWidth: '680px', margin: '0 auto', paddingTop: messages.length ? '32px' : '0' }}>
              {!messages.length && <WelcomeScreen onSelectTopic={handleSend} />}
              {messages.map((msg, i) => {
                if (msg.type === 'user')     return <UserBubble   key={i} text={msg.text} />
                if (msg.type === 'response') return <ResponseCard  key={i} data={msg.data} onFollowUp={handleSend} />
                if (msg.type === 'error')    return <ErrorMessage  key={i} message={msg.message} onRetry={() => handleRetry(msg.retryText)} />
                return null
              })}
